...
// 감정라벨
function getEmotionLabel(count, asHtml = false) {
function getEmotionLabel(count, asHtml = false, size = 16) {
  const c = normalizeEmotion(count);
  let paws;
  if (asHtml) {
    paws = `<img src="${PAW_FOOT_ICON_SRC}" alt="paw" class="paw-foot-icon" />`.repeat(c);
...
function renderList() {
  ...
  filtered.forEach((item) => {
    ...
    const emoLabel = getEmotionLabel(item.emotion, true);
...
function applyCatProfileToUI(profile) {
  ...
  previewText.textContent = `${pose} · ${fur} · ${eyes} eyes · ${acc} · ${scene}`;
...
function renderPawStats(period) {
  ...
  for (let value = 1; value <= 5; value++) {
    ...
    valueLabel.setAttribute("aria-label", `${value} paws`); // Use innerHTML to render the image tags
    valueLabel.innerHTML = `<img src="${PAW_FOOT_ICON_SRC}" alt="" class="paw-foot-icon" aria-hidden="true" />`.repeat(
      value
    );
