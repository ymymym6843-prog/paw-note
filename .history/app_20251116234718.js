...
/* e:\vibecoding\diary_re\style.css 에 추가 */
...
.cat-controls-grid .control-row label {
  font-weight: 500;
  font-size: 0.9rem;
  text-align: right;
...
.cat-option-btn .cat-option-label {
  z-index: 2;
}

/* 컬러칩 스타일 */
.cat-option-btn.color-chip {
  width: 32px;
  height: 32px;
  border-radius: 50%;
  padding: 0;
...
.cat-option-btn.color-chip .cat-option-label {
  display: none; /* 툴팁 등을 위해 남겨둘 수 있으나, 여기선 숨김 */
}

.cat-option-btn.color-chip::before {
  content: '';
  position: absolute;
  top: 50%;
  left: 50%;
  ...
  background-image: linear-gradient(45deg, rgba(255,255,255,0.2), rgba(0,0,0,0.1));
  border: 1px solid rgba(0,0,0,0.1);
}

/* 무지개 컬러칩 특별 스타일 */
.cat-option-btn.color-chip[data-value="cushion_rainbow"]::before {
...
.cat-option-btn.color-chip[data-value="cushion_rainbow"]::before {
  ...
  cushion: ["none", "cushion_blue", "cushion_rainbow", "cushion_red", "cushion_yellow", "cushion_orange", "cushion_green"],
