...
function getEmotionMeta(value) {
  if (value === "happy") return { label: "😊 행복", cls: "happy" };
  if (value === "sad") return { label: "😢 슬픔", cls: "sad" };
  return { label: "😐 보통", cls: "neutral" };
}
...
function renderList() {
  ...
  const filtered = diaries.filter((item) => {
    ...
    const emotionMeta = getEmotionMeta(item.emotion || "neutral");
    const text = (item.content || "") + " " + dateText + " " + emotionMeta.label;
  ...
  filtered.forEach((item) => {
    ...
    btn.addEventListener("click", () => {
      ...
      emotionSelect.value = item.emotion || "neutral";
